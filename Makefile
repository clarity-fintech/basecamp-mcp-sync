...
check:
	...
	$(MISC) clrty/basecamp-mcp-sync.mis --check --compact-letters
	$(MISC) mis/kernel/BasecampMcpSyncKernel.mis --check --compact-letters
	@find mis -name '*.mis' | while read f; do $(MISC) "$$f" --check --compact-letters || exit 1; done
	@echo OK basecamp-mcp-sync
...
network-connect: commands-check
	...
	@echo MODULES mis/commands/BasecampMcpSyncCommands.mis
